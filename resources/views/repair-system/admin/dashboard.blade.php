                ...
                {{-- @json($repair->logs, JSON_PRETTY_PRINT) --}}
                    <tr>
                        ...
                        <td class="text-center">
                            @if($repair->status !== 'rejected' && $repair->status !== 'cancel')
                                <button class="btn btn-sm btn-outline-secondary" onclick="adminUpdateRepair({{ $repair->id }},'{{ $repair->priority }}','{{ $repair->title }}', '{{ $repair->status }}', '{{ optional($repair->latestLog)->note }}', '{{ $repair->description }}')">
                                    แก้ไข
                                </button>
