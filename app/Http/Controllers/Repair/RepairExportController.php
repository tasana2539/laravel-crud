...
class RepairExportController extends Controller
{
    ...
    public function requestAllTask(Request $request)
    {
        // เก็บ ID ลง Session
        session(['all_task_repair_id' => $request->repair_id]);

        // Redirect ไปแสดง PDF โดยไม่โชว์ id
        return redirect()->route('repair.pdf.all-view');
    }

    public function viewAllTask(Request $request)
    {
        $id = session('all_task_repair_id');
        if (!$id) {
            abort(403, 'logs task access denied');
        }

        $repair = Repair::with(['logs'])->get();
